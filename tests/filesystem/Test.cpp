...
TEST_CASE("watch for file add", "[added]") {
	const auto test_folder_path = std::filesystem::path("./");
	const auto test_file_name = std::filesystem::path("test.txt");

	std::promise<test_string> promise;
	std::future<test_string> future = promise.get_future();
	filewatch::FileWatch<std::filesystem::path> watch(test_folder_path, [&promise](const std::filesystem::path& path, const filewatch::Event change_type) {
